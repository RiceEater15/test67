...
actorsEl.innerHTML = cast.map(a => `<div class="actor-card"> <img src="${a.profile_path ? 'https://image.tmdb.org/t/p/w185' + a.profile_path : 'https://placehold.co/185x278/222/666?text=?'}" alt="${a.name}" loading="lazy" /> <div class="actor-info"><h5>${a.name}</h5><p>${a.character}</p></div> </div>`).join(’’);
} catch {
