...
section.innerHTML = `<div class="row-header"> <div class="row-title" onclick="goSeeMore('${cat.endpoint}','${cat.name}')" title="See all"> ${cat.name} <span class="arrow">Explore all <i class="bi bi-chevron-right"></i></span> </div> <span class="see-all" onclick="goSeeMore('${cat.endpoint}','${cat.name}')">See All</span> </div> <div class="scroll-track-outer"> <button class="scroll-btn left" onclick="scrollRow('${cleanName}', -1)"><i class="bi bi-chevron-left"></i></button> <div class="scroll-track" id="${cleanName}"></div> <button class="scroll-btn right" onclick="scrollRow('${cleanName}', 1)"><i class="bi bi-chevron-right"></i></button> </div>`;
...
card.innerHTML = `<div class="card-inner"> <img src="${poster}" alt="${title}" loading="lazy" /> <div class="card-overlay"> <div class="card-title-hover">${title}</div> <div class="card-actions"> <div class="card-btn play" title="Play"><i class="bi bi-play-fill"></i></div> <div class="card-btn fav-heart" title="Favorite">${isFav ? '❤️' : '🤍'}</div> ${rating ?`<span class="card-rating">★ ${rating}</span>`: ''} </div> </div> </div>`;
