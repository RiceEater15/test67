...
card.innerHTML = `<img src="${poster}" alt="${title}" loading="lazy" /> <div class="card-overlay"> <div class="card-title">${title}</div> <div class="card-meta"> ${rating ?`<span class="card-rating">★ ${rating}</span>`: ''} ${release ?`<span>${release.slice(0, 4)}</span>`: ''} </div> </div>`;
